...
import { createSente, type ToolPart } from "@sente-ai/sdk"
...
console.log("🚀 Starting sente server...")
const sente = await createSente({
  port: 0,
})
console.log("✅ Sente server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await sente.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new sente session...")
    const { client, server } = sente
    ...
    console.log("✅ Created sente session:", createResult.data.id)
  ...
  console.log("📝 Sending to sente:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Sente response:", JSON.stringify(result, null, 2))
