...
This is the living design and status document for `@sente-ai/codemode` and its existing V2 Sente adapter.
...
`packages/sente/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
