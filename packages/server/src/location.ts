...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-sente-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-sente-directory"] ? decode(request.headers["x-sente-directory"]) : process.cwd())
