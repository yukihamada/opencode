...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for sente serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SenteHttpApi)
...
export const PublicApi = SenteHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "sente",
    version: "1.0.0",
    description: "sente api",
