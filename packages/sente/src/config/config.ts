...
import type { ConsoleState } from "@sente-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@sente-ai/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@sente/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["sente.jsonc", "sente.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://teai.io/sente/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SENTE_CONFIG && !Flag.SENTE_CONFIG_DIR && !Flag.SENTE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://teai.io/sente/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "sente.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "sente.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://teai.io/sente/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SENTE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/sente`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://teai.io/sente/config.json"
        ...
        if (Flag.SENTE_CONFIG) {
          yield* merge(Flag.SENTE_CONFIG, yield* loadFile(Flag.SENTE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SENTE_CONFIG })
        }

        if (!Flag.SENTE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("sente", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SENTE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SENTE_CONFIG_DIR", { path: Flag.SENTE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".sente") || dir === Flag.SENTE_CONFIG_DIR) {
            for (const file of ["sente.json", "sente.jsonc"]) {
          ...
          // Auto-discovered plugins under `.sente/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.SENTE_CONFIG_CONTENT) {
          const source = "SENTE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.SENTE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from SENTE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["SENTE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("SENTE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SENTE_DISABLE_AUTOCOMPACT) {
