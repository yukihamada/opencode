...
const TUI_SCHEMA_URL = "https://teai.io/sente/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from sente.json files
...
async function senteFiles(input: { directories: string[]; cwd: string }) {
  ...
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "sente"))
  }
  if (Flag.SENTE_CONFIG) files.push(Flag.SENTE_CONFIG)
