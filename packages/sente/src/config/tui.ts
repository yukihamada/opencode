...
import { isRecord } from "@sente-ai/tui/util/record"
...
export class Service extends Context.Service<Service, Interface>()("@sente/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old sente.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.sente`
  // folders between cwd and home, and SENTE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SENTE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SENTE_TUI_CONFIG override, if set.
  if (Flag.SENTE_TUI_CONFIG) {
    const configFile = Flag.SENTE_TUI_CONFIG
  ...
  // 4. `.sente` directories (and SENTE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".sente") || dir === Flag.SENTE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".sente") && dir !== Flag.SENTE_CONFIG_DIR) continue
