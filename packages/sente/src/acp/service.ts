...
function defaultModelFromConfig(
  ...
  // a default. Configured model, sente provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const senteProvider = providers[ProviderV2.ID.make("sente")]
  const senteModel = senteProvider ? Provider.sort(Object.values(senteProvider.models))[0] : undefined
  if (senteProvider && senteModel) return { providerID: senteProvider.id, modelID: senteModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Sente service failure", service })
}
