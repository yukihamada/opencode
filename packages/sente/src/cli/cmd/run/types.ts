// Shared type vocabulary for the direct interactive mode (`sente --mini`).
//
...
import type { SenteClient, PermissionRequest, QuestionRequest, ToolPart } from "@sente-ai/sdk/v2"
...
type PromptModel = Parameters<SenteClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<SenteClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<SenteClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<SenteClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<SenteClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<SenteClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<SenteClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<SenteClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<SenteClient["question"]["reject"]>[0]
