...
// CLI entry point for `sente run` and `sente --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`sente --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`sente --mini --attach`): connects to a running
//      sente server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run sente with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running sente server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SENTE_SERVER_USERNAME or 'sente')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function share(sdk: SenteClient, sessionID: string) {
      ...
      async function current(sdk: SenteClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: SenteClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: SenteClient) {
        if (!args.agent) return undefined
        if (args.attach) {
