...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@sente-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@sente-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@sente-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@sente-ai/tui/context/theme"
import { Global } from "@sente-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@sente-ai/core/util/flock"
import { Flag } from "@sente-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@sente-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@sente-ai/core/v1/config/plugin"
import { createCommandShim } from "@sente-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@sente-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".sente"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".sente", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".sente", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SENTE_PURE ? [] : pluginOrigins
    if (Flag.SENTE_PURE && pluginOrigins.length) {
    }
