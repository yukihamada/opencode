...
- `native-request.ts` converts opencode's normalized session input into a native `@sente-ai/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    native-request.ts       opencode/AI SDK-shaped input -> @sente-ai/llm LLMRequest
...
`native-runtime.ts` evaluates the gate and either bridges into `@sente-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.
