...
const USER_AGENT = `sente/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const opencodeProjectID = input.model.providerID.startsWith("sente")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("sente")
        ? {
            ...(opencodeProjectID ? { "x-sente-project": opencodeProjectID } : {}),
            "x-sente-session": input.sessionID,
            "x-sente-request": input.user.id,
            "x-sente-client": input.flags.client,
