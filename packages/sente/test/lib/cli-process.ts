// Subprocess test harness for the sente CLI. Spawns the real binary against
...
//
// Configuration flows through sente's built-in test affordances:
//   - SENTE_CONFIG_CONTENT      : provider config inline, no files to find
//   - SENTE_TEST_HOME           : pins os.homedir() → tmpdir
//   - SENTE_DISABLE_PROJECT_CONFIG : skip walking up for sente.json
//   - SENTE_PURE                : skip external plugin discovery + install
//   - SENTE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `sente.run` is fully wired. The shape supports adding more
// builders (`sente.serve(opts)`, `sente.acp(opts)`, `sente.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of SenteCli.
...
const senteRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(senteRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    SENTE_CONFIG_CONTENT: configJson,
    SENTE_DISABLE_PROJECT_CONFIG: "1",
    SENTE_PURE: "1",
...
// Typed equivalent of constructing argv for `sente run`. New flags should
...
// `sente serve` is a long-lived process — it never exits on its own.
...
// `sente acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type SenteCli = {
  ...
  // Spawn `sente serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `sente acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("sente.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          SENTE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("sente.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("sente.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "sente server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `sente serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("sente.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const sente: SenteCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, sente })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `sente.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "sente") {
...
// every call site. Use this for any test that needs the sente CLI fixture.
//
...
// resources (e.g. `sente.serve`) without an extra `Effect.scoped` wrapper —
