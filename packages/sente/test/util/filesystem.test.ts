...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "sente.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "sente.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sente.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sente.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["sente.json", "sente.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "sente.json"),
        path.join(project, "sente.jsonc"),
      ])
    })
  })
