...
`,
  ...
  const pure = process.env.SENTE_PURE
  const meta = process.env.SENTE_PLUGIN_META_FILE
  process.env.SENTE_PURE = "1"
  process.env.SENTE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SENTE_PURE
    } else {
      process.env.SENTE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SENTE_PLUGIN_META_FILE
    } else {
      process.env.SENTE_PLUGIN_META_FILE = meta
    }
  }
})
