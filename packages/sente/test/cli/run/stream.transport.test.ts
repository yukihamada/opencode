...
import { SenteClient, type GlobalEvent } from "@sente-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<SenteClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<SenteClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<SenteClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<SenteClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<SenteClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: SenteClient["event"]["subscribe"]
    globalEvent?: SenteClient["global"]["event"]
    promptAsync?: SenteClient["session"]["promptAsync"]
    status?: SenteClient["session"]["status"]
    messages?: SenteClient["session"]["messages"]
    children?: SenteClient["session"]["children"]
    permissions?: SenteClient["permission"]["list"]
    questions?: SenteClient["question"]["list"]
  } = {},
) {
  const client = new SenteClient()

  const subscribe: SenteClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: SenteClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: SenteClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: SenteClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: SenteClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: SenteClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: SenteClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: SenteClient["question"]["list"] = input.questions ?? (() => ok([]))
