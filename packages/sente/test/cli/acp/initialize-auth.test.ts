...
describe("sente acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ sente }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ sente }))
        ...
        expect(initialized.agentInfo?.name).toBe("Sente")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ sente }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ sente })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("sente-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "sente-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.SENTE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ sente }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("sente-login")
