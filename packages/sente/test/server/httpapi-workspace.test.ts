      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-sente-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://teai.io/sente/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-sente-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-sente-workspace")
