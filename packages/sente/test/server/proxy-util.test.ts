...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips sente-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-sente-directory": "/home/user/project",
          "x-sente-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-sente-directory")).toBeNull()
      expect(result.get("x-sente-workspace")).toBeNull()
