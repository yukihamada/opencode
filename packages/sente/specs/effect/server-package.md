...
Practical reference for a future `packages/server` split after the sente
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/sente`.
...
Until enough shared service code lives outside `packages/sente`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/sente`

It should not import `packages/sente` services while `packages/sente`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on sente-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/sente` implementation details.
