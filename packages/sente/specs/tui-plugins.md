...
```json
{
  ...
  "plugin": ["@acme/sente-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- The value is a semver range checked against the running Sente version.
...
- `sente plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `sente plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.sente` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.sente`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.sente`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.sente` dir or `SENTE_CONFIG_DIR`, Sente installs `@sente-ai/plugin` into that dir and writes:
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Sente's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Sente registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `sente.default`.
...
- Local plugins persist installed themes under the local `.sente/themes` area near the plugin config source.
...
- Local smoke plugin: `.sente/plugins/tui-smoke.tsx`
- Local vim plugin: `.sente/plugins/tui-vim.tsx`
- Local smoke config: `.sente/tui.json`
- Local smoke theme: `.sente/plugins/smoke-theme.json`
