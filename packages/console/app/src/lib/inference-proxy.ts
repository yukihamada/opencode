...
export async function proxyInference(
  ...
  const requestID = request.headers.get("x-sente-request-id") ?? request.headers.get("x-sente-request")
  if (requestID) forwarded.headers.set("x-sente-request-id", requestID)
