...
const dataPath = "/data"
const statsCacheParam = "__sente_stats_locale"
...
export async function statsProxy(evt: APIEvent) {
  ...
  targetUrl.hostname = Resource.App.stage === "production" ? "stats.teai.io/sente" : "stats.teai.io/sente/dev"
  targetUrl.port = ""
...
function withStatsCacheStatus(response: Response, status: "HIT" | "MISS") {
  const headers = new Headers(response.headers)
  headers.set("x-sente-stats-cache", status)
