import type { TuiPluginApi } from "@sente-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const tip = createMemo(() => {
    ...
    return tips[tipIndex() % tips.length] ?? NO_MODELS_TIP
  }, NO_MODELS_TIP)
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public teai.io/sente link",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.sente/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.sente/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.sente/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Sente from reading sensitive files",
  "Use {highlight}sente run{/highlight} for non-interactive scripting",
  "Use {highlight}sente --continue{/highlight} to resume the last session",
  "Use {highlight}sente run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}sente serve{/highlight} for headless API access to Sente",
  "Use {highlight}sente run --attach{/highlight} to connect to a running server",
  "Run {highlight}sente upgrade{/highlight} to update to the latest version",
  "Run {highlight}sente auth list{/highlight} to see all configured providers",
  "Run {highlight}sente agent create{/highlight} for guided agent creation",
  "Use {highlight}/sente{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}sente github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/sente fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.sente/themes/{/highlight} directory",
  ...
  "Run {highlight}sente debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/sente{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with Sente Zen for curated, tested models",
