...
export const SenteKeymapProvider = KeymapProvider
export const useSenteKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type SenteModeStack = ReturnType<typeof createSenteModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, SenteModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createSenteModeStack(keymap: OpenTuiKeymap) {
...
export function useSenteModeStack() {
  return getSenteModeStack(useSenteKeymap())
}

export function getSenteModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Sente mode stack is not registered for this keymap")
  return value
}
...
export function registerSenteKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createSenteModeStack(keymap)
