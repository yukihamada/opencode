...
import type { useSenteKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useSenteKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getSenteModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getSenteModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
