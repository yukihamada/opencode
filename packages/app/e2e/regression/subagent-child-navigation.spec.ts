...
import { currentSession, mockSenteServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Sente/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#sente-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "sente", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("sente.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
