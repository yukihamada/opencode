...
export function wslSenteAction(check?: WslSenteCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Sente"
  if (check.matchesDesktop === false) return "Update Sente"
}
...
export function addServerViewModel(input: {
  ...
  const senteCheck = selectedDistro ? (state?.senteChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.senteChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateSente" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.senteMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installSente" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingSente =
    input.state?.job?.kind === "install-sente" && input.state.job.distro === input.selectedDistro
  if (!ready || probingSente) {
    return {
      variant: "contrast",
  ...
  if (!addServerSenteReady(input.senteCheck)) {
    const update = !!input.senteCheck?.resolvedPath && input.senteCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingSente
        ? { key: "wsl.onboarding.updatingSente" }
        : update
          ? { key: "wsl.onboarding.updateSente" }
          : { key: "wsl.onboarding.installSente" },
      disabled: !!input.state?.job || input.adding,
      action: "install-sente",
      loading: installingSente,
...
function addServerSenteReady(check: WslSenteCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.senteChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.senteChecks[item.name]) return [`sente:${item.name}`]
    return []
  })
