...
import { Tag } from "@sente-ai/ui/v2/badge-v2"
...
import { wslSenteAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.senteChecks[item.config.distro]
          const senteAction = () => wslSenteAction(check())
          const busy = () => wsl.data?.job?.kind === "install-sente" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={senteAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installSente(item.config.distro))}
                    >
