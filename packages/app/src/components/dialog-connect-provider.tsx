import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@sente-ai/client/promise"
import { Button } from "@sente-ai/ui/button"
import { useDialog } from "@sente-ai/ui/context/dialog"
import { Dialog } from "@sente-ai/ui/dialog"
import { Icon } from "@sente-ai/ui/icon"
import { IconButton } from "@sente-ai/ui/icon-button"
import { List, type ListRef } from "@sente-ai/ui/list"
import { ProviderIcon } from "@sente-ai/ui/provider-icon"
import { Spinner } from "@sente-ai/ui/spinner"
import { Tag } from "@sente-ai/ui/tag"
import { TextField } from "@sente-ai/ui/text-field"
import { ButtonV2 } from "@sente-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@sente-ai/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "sente-go") return language.t("dialog.provider.senteGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "sente"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.sente.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["sente", "sente-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "sente" || provider.id === "sente-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "sente"
                                ? "dialog.provider.sente.tagline"
                                : "dialog.provider.senteGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "sente" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "sente-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "sente"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.senteZen.line1")}</div>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "sente"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.senteZen.line1")}</div>
