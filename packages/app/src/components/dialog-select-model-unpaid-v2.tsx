...
import { useTheme } from "@sente-ai/ui/theme"
...
const featuredProviders = ["sente", "sente-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "sente" && (!item.cost || item.cost.input === 0)
