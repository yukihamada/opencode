...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("sente://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("sente://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("sente://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("sente://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("sente://open-project")).toBeUndefined()
    expect(parseDeepLink("sente://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "sente://open-project?directory=/a",
      "sente://other?directory=/b",
      "sente://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("sente://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("sente://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("sente://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("sente://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "sente://new-session?directory=/a",
      "sente://open-project?directory=/b",
      "sente://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SENTE__: {
        deepLinks: ["sente://open-project?directory=/a"],
      },
    } as unknown as Window & { __SENTE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["sente://open-project?directory=/a"])
