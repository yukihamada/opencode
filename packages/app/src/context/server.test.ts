  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://sente.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://sente.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://sente.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://sente.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://sente.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://sente.example.com"),
      ),
    ).toEqual({
