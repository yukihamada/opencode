...
import type { retry } from "@sente-ai/core/util/retry"
import type { SenteEvent, SessionApi } from "@sente-ai/client/promise"
import type { Message, SenteClient, Part, Session } from "@sente-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as SenteEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as SenteClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
