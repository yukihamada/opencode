...
import { ConfigProvider } from "@sente-ai/core/config/provider"
import { AppNodeBuilder } from "@sente-ai/core/effect/app-node-builder"
import { LayerNode } from "@sente-ai/core/effect/layer-node"
import { ConfigMigrateV1 } from "@sente-ai/core/v1/config/migrate"
...
import { Policy } from "@sente-ai/core/policy"
import { Project } from "@sente-ai/core/project"
...
describe("Config", () => {
  ...
  it.live("loads sente JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "sente.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "sente.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "sente.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "sente.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "sente.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "sente.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .sente configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".sente"), { recursive: true })
            await fs.mkdir(path.join(directory, ".sente"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "sente.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "sente.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "sente.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "sente.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "sente.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".sente", "sente.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".sente", "sente.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".sente")),
              AbsolutePath.make(path.join(directory, ".sente")),
            ])
