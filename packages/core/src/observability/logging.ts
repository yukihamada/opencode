...
export function fileLogger(file = path.join(Global.Path.log, "sente.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.SENTE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.SENTE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
