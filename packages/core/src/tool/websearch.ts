...
export class ConfigService extends Context.Service<ConfigService, Config>()("@sente/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SENTE_WEBSEARCH_PROVIDER === "exa" || process.env.SENTE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SENTE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SENTE_EXPERIMENTAL") || truthy("SENTE_ENABLE_EXA") || truthy("SENTE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SENTE_ENABLE_PARALLEL") || truthy("SENTE_EXPERIMENTAL_PARALLEL"),
