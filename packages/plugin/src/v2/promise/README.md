# Sente V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@sente-ai/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an Sente extension point.
...
```ts
import { define } from "@sente-ai/plugin/v2/promise"
