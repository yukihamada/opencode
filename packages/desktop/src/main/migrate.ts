...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.sente.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `sente.settings.dat` is special: it maps to the `sente.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"sente.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // sente.settings.dat → the electron settings store ("sente.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "sente.global.dat").
  const storeName = filename === "sente.settings.dat" ? "sente.settings" : filename
