...
let releaseSenteResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectSenteVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectSenteVersion("1.14.35", "1.16.2")).toThrow(
    "Sente update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Sente", () => {
  expect(
    wslServerIdToRestart(
      [
        {
  ...
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.sente/bin/sente",
          version: "1.16.2",
...
test("ignores stale background Sente checks after removing a WSL server", async () => {
  persistedServers = []
  releaseSenteResolve = undefined
  ...
  await waitFor(() => !!releaseSenteResolve)
...
test("ignores stale startup Sente checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Sente", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveSente: async (distro) => {
      sente.push(distro)
      return "/home/me/.sente/bin/sente"
    },
  })
  ...
  expect(sente).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().senteChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Sente in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const sente: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
