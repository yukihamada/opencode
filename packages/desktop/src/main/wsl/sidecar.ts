...
import { type WslCommandLine, resolveWslSente, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const sente = await resolveWslSente(distro)
  if (!sente) throw new Error(`Sente is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "sente"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export SENTE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SENTE_CLIENT=desktop",
    `export SENTE_SERVER_USERNAME=${shellEscape(username)}`,
    `export SENTE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(sente)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
