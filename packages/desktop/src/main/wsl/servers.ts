...
import { expectSenteVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveSente?: typeof resolveWslSente
...
export function createWslServersController(
  ...
  const setSenteCheck = (distro: string, check: WslSenteCheck) => {
    setState({
      senteChecks: {
        ...state.senteChecks,
        [distro]: check,
  ...
  const checkSente = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveSente ?? resolveWslSente)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return senteCheck(distro, resolved, version, appVersion)
  }

  const refreshSenteCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setSenteCheck(distro, await checkSente(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const senteChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.senteChecks[distro])
        .map(async (distro) => [distro, await checkSente(distro, opts)] as const),
    )
    if (senteChecks.length) {
      setState({ senteChecks: { ...state.senteChecks, ...Object.fromEntries(senteChecks) } })
    }
  }
  ...
  const refreshSenteCheckBackground = (id: string, distro: string) => {
    void checkSente(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setSenteCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl sente check failed", { id, distro, message })
      })
  }

  const refreshSenteChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkSente(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setSenteCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl sente check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshSenteCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installSente(name: string) {
      await runJob({ kind: "install-sente", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslSente(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Sente installation failed")
        }
        await refreshSenteCheck(name, { signal: abort.signal })
        expectSenteVersion(state.senteChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.senteChecks, distro) : {}),
      })
    },
...
function senteCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "sente is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "sente is installed but could not run",
    }
  }
  return {
    distro,
