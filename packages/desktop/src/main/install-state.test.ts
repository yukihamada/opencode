...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Sente launch", () => {
    expect(hasExistingAppState([file("sente.settings")])).toBe(true)
    expect(hasExistingAppState([file("sente.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("sente")])).toBe(true)
  })
})
