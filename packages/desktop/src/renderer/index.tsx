...
import type { UpdaterState } from "@sente-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@sente-ai/ui/logo"
import { useTheme } from "@sente-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.SENTE_CHANNEL === "prod" &&
...
const deepLinkEvent = "sente:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__SENTE__ ??= {}
  const pending = window.__SENTE__.deepLinks ?? []
  window.__SENTE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `sente.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://teai.io/sente/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("sente.global.dat").getItem("language")
