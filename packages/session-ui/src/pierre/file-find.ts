...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("sente-find")
  api.delete("sente-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("sente-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("sente-find", new Highlight(...rest))
    return true
  }
