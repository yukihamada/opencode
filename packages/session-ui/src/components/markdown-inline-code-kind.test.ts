...
describe("inlineCodeKind", () => {
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/sente`)).toBe("path")
    expect(inlineCodeKind(`@sente-ai/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://teai.io/sente/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/sente`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://teai.io/sente/docs`)).toBeUndefined()
  })
})
