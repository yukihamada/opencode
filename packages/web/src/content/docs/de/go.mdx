...
Sente Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Sente. Du abonnierst Sente Go und
...
Sente Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Sente Go funktioniert wie jeder andere Provider in Sente.

1. Du meldest dich bei **<a href={console}>Sente Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Sente Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Sente Go abonnieren.
:::
...
Sente Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Sente Config verwendet das Format `sente-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `sente-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Sente Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Sente zu nutzen.
