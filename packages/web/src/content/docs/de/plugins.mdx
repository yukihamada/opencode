---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Sente zu erweitern.
---

Mit Plugins können Sie Sente erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Sente zu ändern.
...
- `.sente/plugins/` – Plugins auf Projektebene
- `~/.config/sente/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/sente/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/sente/sente.json`)
2. Projektkonfiguration (`sente.json`)
3. Globales Plugin-Verzeichnis (`~/.config/sente/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.sente/plugins/`)
...
Sente führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Sente-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Sente erweitern können.
...
:::note
Wenn Sie die Sente-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Sente `.env`-Dateien liest:
...
Plugins können Sente auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Sente aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Sente zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://teai.io/sente/docs/sdk).
