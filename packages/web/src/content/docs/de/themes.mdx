...
Mit Sente kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Sente das `sente`-Theme.
...
Sente liefert mehrere eingebaute Themes mit.
...
- Sente optisch ans Terminal anpassen wollen
...
Sente unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/sente/themes/*.json` oder `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.sente/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.sente/themes/*.json`
