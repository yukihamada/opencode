---
title: Regeln
description: Lege eigene Anweisungen fuer sente fest.
---

Du kannst sente ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in sente den Befehl `/init` aus.
...
So kann sente sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht sente sie zu erweitern.
...
sente kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/sente/AGENTS.md` speichern.
Diese gelten in allen sente-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Sente dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/sente/AGENTS.md` existiert)
...
Beim Start sucht sente in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/sente/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `sente.json` oder in `~/.config/sente/sente.json` angeben.
...
sente parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit sente.json

Empfohlen ist das `instructions`-Feld in der `sente.json`:
...
Du kannst sente explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `sente.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
