...
Wenn Sente Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\sente\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `sente --log-level DEBUG`.

---

## Speicher

sente speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/sente/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\sente`
...
Sente Desktop startet im Hintergrund einen lokalen Sente-Server (`sente-cli`-Sidecar).
...
- Nur macOS: `Sente`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/sente/sente.jsonc` (oder `~/.config/sente/sente.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/sente/sente.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\sente\sente.jsonc`
...
Sente kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/sente/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\sente\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Sente ihn neu aufbauen kann.

1. Beende Sente Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/sente`
- **Linux**: loesche `~/.cache/sente` (oder fuehre aus: `rm -rf ~/.cache/sente`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\sente`

3. Starte Sente Desktop neu.

---

### Server-Verbindungsprobleme beheben

Sente Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `sente.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `SENTE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `SENTE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Sente Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Sente-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Sente Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Sente in den OS-Einstellungen aktiviert sind, und
...
1. Beende Sente Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Sente Desktop):

- `sente.settings.dat` (desktop default server URL)
- `sente.global.dat` und `sente.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Sente hast:
...
3. Stelle sicher, dass du die neueste Version hast: `sente upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `sente models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\sente`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. sente installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\sente`

2. Starte sente neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt sente, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
sente erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
