---
title: Server
description: Nutze den sente-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `sente serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein sente-Client nutzen kann.
...
Setze `SENTE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `sente`, kann aber mit `SENTE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `sente serve` und `sente web`.
...
Wenn du `sente` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den sente-Server, um sente programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `sente serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `sente serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Sente-[IDE](/docs/ide)-Plugins.
...
Der sente-Server stellt folgende APIs bereit.
