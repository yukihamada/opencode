...
Agent Skills erlauben Sente, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Sente sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Sente vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.sente/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.sente/skills/git-release/SKILL.md` zum Beispiel so:
...
Sente listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `sente.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `sente.json`):
