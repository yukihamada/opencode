...
You can add external tools to Sente using the _Model Context Protocol_, or MCP. Sente supports both local and remote servers.
...
You can define MCP servers in your [Sente Config](https://teai.io/sente/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/sente` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Sente automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Sente will:
...
If the server requires authentication, Sente will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `sente mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Sente will store the tokens securely in `~/.local/share/sente/mcp-auth.json`.
...
Your MCPs are available as tools in Sente, alongside built-in tools. So you can manage them through the Sente config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Sente to your Sentry account.
