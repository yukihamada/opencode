...
Avec Sente, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Sente utilise notre propre thème `sente`.
...
Sente est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Sente corresponde à l'apparence de leur terminal
...
Sente prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/sente/themes/*.json` ou `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.sente/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.sente/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/sente/themes
vim ~/.config/sente/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .sente/themes
vim .sente/themes/my-theme.json
```
...
{
  "$schema": "https://teai.io/sente/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
