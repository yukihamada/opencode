---
title: SDK
description: Client JS de type sécurisé pour le serveur sente.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK sente JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler sente par programme.
...
Créez une instance de sente :

```javascript
import { createSente } from "@sente-ai/sdk"

const { client } = await createSente()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `sente.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${sente.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de sente, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@sente-ai/sdk"
```
