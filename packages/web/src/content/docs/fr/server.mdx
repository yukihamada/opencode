---
title: Serveur
description: Interagissez avec le serveur sente via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `sente serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client sente peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `sente.local` |
...
Définissez `SENTE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `sente`, ou définissez `SENTE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `sente serve` et à `sente web`.
...
Lorsque vous exécutez `sente`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur sente pour interagir avec sente par programmation.
:::

Cette architecture permet à sente de prendre en charge plusieurs clients et vous permet d'interagir avec sente par programme.

Vous pouvez exécuter `sente serve` pour démarrer un serveur autonome. Si vous avez le
sente TUI en cours d'exécution, `sente serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Sente [IDE](/docs/ide).
...
Le serveur sente expose les API suivantes.
