---
title: Plugins
description: Écrivez vos propres plugins pour étendre Sente.
---

Les plugins vous permettent d'étendre Sente en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Sente.
...
- `.sente/plugins/` - Plugins au niveau du projet
- `~/.config/sente/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/sente/node_modules/`.
...
1. Configuration globale (`~/.config/sente/sente.json`)
2. Configuration du projet (`sente.json`)
3. Répertoire global des plugins (`~/.config/sente/plugins/`)
4. Répertoire des plugins du projet (`.sente/plugins/`)
...
Sente exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK sente pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre sente.
...
:::note
Si vous utilisez l'application de bureau Sente, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez sente de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à sente :
...
L'assistant `tool` crée un outil personnalisé que sente peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour sente aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://teai.io/sente/docs/sdk) pour plus de détails.
