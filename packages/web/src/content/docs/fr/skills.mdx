...
Les compétences d'agent permettent à Sente de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Sente recherche ces emplacements :

- Configuration du projet : `.sente/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/sente/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Sente remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.sente/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.sente/skills/git-release/SKILL.md` comme ceci :
...
Sente répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `sente.json` :
...
**Pour les agents intégrés** (dans `sente.json`) :
