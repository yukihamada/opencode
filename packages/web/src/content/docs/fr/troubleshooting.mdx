...
Pour déboguer les problèmes avec Sente, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\sente\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `sente --log-level DEBUG`.

---

## Stockage

sente stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/sente/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\sente`
...
Sente Desktop exécute un serveur Sente local (le side-car `sente-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Sente` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/sente/sente.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\sente\sente.jsonc`
...
Sente peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/sente/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\sente\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Sente puisse le reconstruire.

1. Quittez complètement Sente Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/sente`
- **Linux** : supprimez `~/.cache/sente` (ou exécutez `rm -rf ~/.cache/sente`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\sente`

3. Redémarrez le bureau Sente.

---

### Résoudre les problèmes de connexion au serveur

Sente Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `sente.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `SENTE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `SENTE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Sente Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Sente.

---

### Les notifications ne s'affichent pas

Sente Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Sente dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Sente.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Sente Desktop) :

- `sente.settings.dat` (serveur de bureau par défaut URL)
- `sente.global.dat` et `sente.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Sente :
...
3. Assurez-vous d'avoir la dernière version avec `sente upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `sente models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\sente`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. sente installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\sente`

2. Redémarrez sente pour réinstaller les derniers packages du fournisseur

Cela forcera sente à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
sente détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
