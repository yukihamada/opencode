...
مع Sente يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Sente سمتنا `sente`.
...
- يريدون أن يطابق Sente مظهر terminal لديهم
...
يدعم Sente نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/sente/themes/*.json` أو `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.sente/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.sente/themes/*.json`
