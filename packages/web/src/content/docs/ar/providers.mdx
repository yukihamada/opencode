---
title: المزوّدون
description: استخدام أي موفّر LLM في Sente.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Sente ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Sente.
...
في `~/.local/share/sente/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Sente.
...
Sente Zen هي قائمة نماذج يوفّرها فريق Sente وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Sente. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Sente Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر sente، ثم انتقل إلى [teai.io/sente/auth](https://teai.io/sente/auth).
...
يعمل مثل أي مزوّد آخر في Sente واستخدامه اختياري بالكامل.

---

## Sente Go

Sente Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Sente، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Sente.

1. شغّل الأمر `/connect` في TUI، واختر `Sente Go`، ثم انتقل إلى [teai.io/sente/auth](https://teai.io/sente/zen).
...
لاستخدام Amazon Bedrock مع Sente:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل sente:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `sente.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Sente غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين sente لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل sente بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات sente.
...
تخزين الرمز في مساحة تخزين مصادقة sente.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Sente نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Sente
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`sente.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [sente-gitlab-auth](https://www.npmjs.com/package/sente-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع sente:
...
لاستخدام Google Vertex AI مع Sente:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر sente، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [sente-helicone-session](https://github.com/H2Shami/sente-helicone-session) لتسجيل كل محادثة Sente تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Sente مدرجة كجلسة مستقلة.
...
يمكنك ضبط sente لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط sente لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط sente لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Sente. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/sente) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Sente:
...
3. انسخ مفتاح API لاستخدامه في Sente.
...
6. **مهم**: قبل استخدام نماذج السحابة في Sente، يجب سحب معلومات النموذج محليا:
...
Sente Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Sente. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Sente Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Sente Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Sente.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات sente.
...
5. يمكنك أيضا تخصيصها عبر إعدادات sente. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Sente:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات sente. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع sente. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `sente.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Sente بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `sente auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات sente وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات sente.
