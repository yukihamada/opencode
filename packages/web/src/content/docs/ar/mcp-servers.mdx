...
يمكنك إضافة أدوات خارجية إلى Sente باستخدام _Model Context Protocol_، أو MCP. يدعم Sente خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Sente](https://teai.io/sente/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/sente` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Sente تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Sente بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Sente المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `sente mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Sente الرموز بشكل آمن في `~/.local/share/sente/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Sente إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Sente مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Sente بحساب Sentry الخاص بك.
