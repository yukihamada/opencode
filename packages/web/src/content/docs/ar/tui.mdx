---
title: TUI
description: استخدام واجهة المستخدم TUI في Sente.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Sente واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Sente إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Sente في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Sente. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Sente. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `sente.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Sente بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `sente.default`.
