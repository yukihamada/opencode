...
تتيح مهارات الوكيل لـ Sente اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Sente في هذه المواقع:

- إعدادات المشروع: `.sente/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/sente/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Sente صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.sente/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/sente/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.sente/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Sente المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `sente.json`:
...
**للوكلاء المدمجين** (في `sente.json`):
