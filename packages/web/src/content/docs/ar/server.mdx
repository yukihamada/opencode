---
title: الخادم
description: تفاعل مع خادم sente عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `sente serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل sente استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `sente.local` |
...
عيّن `SENTE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `sente`، أو عيّن `SENTE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `sente serve` و `sente web`.
...
عند تشغيل `sente` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم sente للتفاعل مع sente برمجيا.
:::

تتيح هذه البنية لـ sente دعم عدة عملاء وتمكّنك من التفاعل مع sente برمجيا.

يمكنك تشغيل `sente serve` لبدء خادم مستقل. إذا كانت واجهة sente في terminal (TUI)
قيد التشغيل، فسيبدأ `sente serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Sente لـ [IDE](/docs/ide).
...
يعرض خادم sente واجهات API التالية.
