...
Sente Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Sente. You subscribe to Sente Go and
...
Sente Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Sente Go works like any other provider in Sente.

1. You sign in to **<a href={console}>Sente Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Sente Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Sente Go.
:::
...
Sente Go includes the following limits:
...
uses the format `sente-go/<model-id>`. For example, for Kimi K3, you would
use `sente-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Sente as well.
