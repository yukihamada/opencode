---
title: Serwer
description: Interakcja z serwerem sente za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `sente serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient sente.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `sente.local` |
...
Ustaw `SENTE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `sente` lub ustaw `SENTE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `sente serve`, jak i `sente web`.
...
Kiedy uruchomisz `sente`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera sente do programowej interakcji z sente.
:::

Ta architektura umożliwia obsługę wielu klientów przez sente i programową interakcję z kodem otwartym.

Możesz uruchomić `sente serve`, aby uruchomić samodzielny serwer. Jeśli masz
sente TUI działa, `sente serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki sente [IDE](/docs/ide).
...
Serwer sente udostępnia następujące interfejsy API.
