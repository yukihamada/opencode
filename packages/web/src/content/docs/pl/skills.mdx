...
Umiejętności agenta pozwalają sente odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
sente przeszukuje te lokalizacje:

- Project config: `.sente/skills/<name>/SKILL.md`
- Global config: `~/.config/sente/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu sente przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.sente/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.sente/skills/git-release/SKILL.md` w ten sposób:
...
sente wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `sente.json`:
...
**Dla agentów wbudowanych** (w `sente.json`):
