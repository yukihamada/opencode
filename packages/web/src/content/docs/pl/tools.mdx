...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Sente posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Sente.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SENTE_EXPERIMENTAL_LSP_TOOL=true` (lub `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Sente lub gdy zmienna środowiskowa `SENTE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Sente:
