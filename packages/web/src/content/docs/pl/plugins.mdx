---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć sente.
---

Wtyczki umożliwiające wyłączenie sente poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania sente.
...
- `.sente/plugins/` - Wtyczki na poziomie projektu
- `~/.config/sente/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/sente/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/sente/sente.json`)
2. Konfiguracja projektu (`sente.json`)
3. Globalny katalog wtyczek (`~/.config/sente/plugins/`)
4. Katalog wtyczek projektu (`.sente/plugins/`)
...
sente uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z sente do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do sente.
...
:::note
Jeśli korzystasz z aplikacji komputerowej sente, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj sente czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do sente:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod sente. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://teai.io/sente/docs/sdk).
