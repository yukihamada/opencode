...
Dzięki sente możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie sente używa naszego własnego motywu `sente`.
...
sente ma kilka wbudowanych motywów.
...
- Chcą, aby Sente pasował do wyglądu ich terminala
...
sente obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/sente/themes/*.json` lub `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.sente/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.sente/themes/*.json`
