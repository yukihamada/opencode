---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w sente.
---

import config from "../../../../config.mjs"
export const console = config.console

sente używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji sente.
...
w `~/.local/share/sente/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim sente
config.
...
Sente Zen to lista modeli dostarczonych przez zespół sente, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z sente. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Sente Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Sente Zen` i przejdź do [teai.io/sente/auth](https://teai.io/sente/zen).
...
Działa jak każdy inny dostawca w sente i jest całkowicie opcjonalny w użyciu.

---

## Sente Go

Sente Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół sente, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z sente.

1. Uruchom polecenie `/connect` w TUI, wybierz `Sente Go` i przejdź do [teai.io/sente/auth](https://teai.io/sente/zen).
...
Aby używać Amazon Bedrock z sente:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania sente:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `sente.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w sente nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować sente do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod sente działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji sente.
...
sente używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować sente
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`sente.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [sente-gitlab-auth](https://www.npmjs.com/package/sente-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem sente:
...
Aby używać Google Vertex AI z sente:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania sente.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą sente, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [sente-helicone-session](https://github.com/H2Shami/sente-helicone-session), aby automatycznie rejestrować każdą konwersację sente jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja sente będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować sente tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować sente tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować sente tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem sente. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/sente).
:::
...
Aby korzystać z Ollama Cloud z sente:
...
3. Skopiuj klucz API do użycia w sente.
...
6. **Ważne**: Przed użyciem modeli chmurowych w sente musisz pobrać informacje o modelu lokalnie:
...
Sente Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół sente. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Sente Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Sente Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API sente.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji sente.
...
5. Możesz je także dostosować za pomocą konfiguracji sente. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z sente:
...
Możesz także dostosować modele za pomocą konfiguracji sente. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z sente. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `sente.json` w katalogu projektu:
...
Pola `limit` pozwalają sente zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `sente auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację sente i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji sente.
