...
Możesz dodać dodatkowe informacje do sente za pomocą _Model Context Protocol_, w skrócie MCP. sente obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [sente Config](https://teai.io/sente/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/sente`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
sente automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, sente:
...
Jeśli serwer wymaga uwierzytelnienia, sente poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `sente mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji sente bezpieczne przechowa tokeny w `~/.local/share/sente/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w sente, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie sente, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć sente z kontem Sentry.
