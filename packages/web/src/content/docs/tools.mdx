...
:::note
This tool is only available when using the OpenCode or OpenCode Go provider, or when either the `SENTE_ENABLE_EXA` or `SENTE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching OpenCode:

```bash
SENTE_ENABLE_EXA=1 opencode
# or
SENTE_ENABLE_PARALLEL=1 opencode
```
