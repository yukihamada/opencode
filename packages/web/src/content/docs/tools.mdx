...
Tools allow the LLM to perform actions in your codebase. Sente comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Sente.
...
:::note
This tool is only available when `SENTE_EXPERIMENTAL_LSP_TOOL=true` (or `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Sente or Sente Go provider, or when either the `SENTE_ENABLE_EXA` or `SENTE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Sente:

```bash
SENTE_ENABLE_EXA=1 sente
# or
SENTE_ENABLE_PARALLEL=1 sente
```
