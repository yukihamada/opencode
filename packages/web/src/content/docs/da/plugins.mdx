---
title: Plugins
description: Skriv dine egne plugins for at udvide Sente.
---

Plugins giver dig mulighed for at udvide Sente ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Sentes standardadfærd.
...
- `.sente/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/sente/node_modules/`.
...
1. Global konfiguration (`~/.config/sente/sente.json`)
2. Projektkonfiguration (`sente.json`)
3. Global plugin-mappe (`~/.config/sente/plugins/`)
4. Projekt plugin bibliotek (`.sente/plugins/`)
...
Sente kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En sente SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide sente.
...
:::note
Hvis du bruger Sente desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå sente i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til sente:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som sente kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for sente sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://teai.io/sente/docs/sdk) for detaljer.
