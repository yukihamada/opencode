...
Agentferdigheter lar Sente oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Sente søker etter disse stedene:

- Prosjektkonfigurasjon: `.sente/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/sente/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Sente opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.sente/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.sente/skills/git-release/SKILL.md` slik:
...
Sente viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `sente.json`:
...
**For innebygde agenter** (i `sente.json`):
