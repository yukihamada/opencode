...
Med Sente kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Sente vårt eget `sente`-tema.
...
Sente støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/sente/themes/*.json` eller `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.sente/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.sente/themes/*.json`
