---
title: TUI
description: Brug af Sente-terminalbrugergrænsefladen.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sente giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Sente starter TUI for den nuværende mappe.
...
Når du bruger Sente TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Sente. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Dette er adskilt fra `sente.json`, som konfigurerer server-/kørselstidsadfærd.
...
- `leader_timeout` - Styrer hvor længe Sente venter efter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `sente.default`.
