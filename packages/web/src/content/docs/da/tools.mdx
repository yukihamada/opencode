...
Her er alle de innebygde verktøyene som er tilgængelige i Sente.
...
:::note
Dette verktøyet er bare tilgængelig når `SENTE_EXPERIMENTAL_LSP_TOOL=true` (eller `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Sente-leverandøren eller når miljøvariabelen `SENTE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
