---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Sente.
---

import config from "../../../../config.mjs"
export const console = config.console

Sente bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Sente-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/sente/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Sente-konfigurationen.
...
Sente Zen er en liste over modeller leveret af Sente-teamet, som er blevet testet og verificeret til at fungere godt med Sente. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Sente Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg sente og gå til [teai.io/sente/auth](https://teai.io/sente/auth).
...
Det fungerer som alle andre udbydere i Sente og er helt valgfrit at bruge.

---

## Sente Go

Sente Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Sente-teamet, som er testet og verificeret til at fungere godt med Sente.

1. Kør kommandoen `/connect` i TUI, vælg `Sente Go`, og gå til [teai.io/sente/auth](https://teai.io/sente/zen).
...
Sådan bruger du Amazon Bedrock med Sente:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører sente:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `sente.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Sente understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere sente til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at sente skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem sente-konfigurationen.
...
gemme tokenet i sente auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Sente bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Sente
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`sente.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [sente-gitlab-auth](https://www.npmjs.com/package/sente-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `sente.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Sente:
...
Sådan bruger du Google Vertex AI med Sente:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem sente, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [sente-helicone-session](https://github.com/H2Shami/sente-helicone-session) plugin for automatisk at logge hver Sente-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Sente-samtale opført som en separat session.
...
Du kan konfigurere sente til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere sente til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere sente til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Sente. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/sente) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Sente:
...
3. Kopier API-nøglen til brug i Sente.
...
6. **Vigtigt**: Før du bruger skymodeller i Sente, skal du hente modelinformationen lokalt:
...
Sente Zen er en liste over testede og verificerede modeller leveret af Sente-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Sente Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Sente Zen**.

   ```txt
   /connect
   ```

3. Indtast Sente API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem sente-konfigurationen.
...
5. Du kan også tilpasse dem gennem sente-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Sente:
...
Du kan også tilpasse modeller gennem sente-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med sente. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `sente.json`-filen i projektkataloget:
...
`limit`-felterne lader Sente forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `sente auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek sente-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i sente-konfigurationen.
