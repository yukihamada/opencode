---
title: Server
description: Interact with sente server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `sente serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an sente client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `sente.local` |
...
Set `SENTE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `sente`, or set `SENTE_SERVER_USERNAME` to override it. This applies to both `sente serve` and `sente web`.
...
When you run `sente` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the sente server to interact with sente programmatically.
:::

This architecture lets sente support multiple clients and allows you to interact with sente programmatically.

You can run `sente serve` to start a standalone server. If you have the
sente TUI running, `sente serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Sente [IDE](/docs/ide) plugins.
...
The sente server exposes the following APIs.
