...
Sente Goは、人気のオープンなコーディングモデルに安定してアクセスできる、低価格の**月額10ドルのサブスクリプション**です。
...
Sente Goを使用すると、これらのモデルに**月額10ドル**でアクセスできます。
...
Sente Goは、[Sente](https://teai.io/sente)や、同様の種類のリクエストを生成するその他の一般的なコーディングエージェントで使用することを想定しています。
...
3\. プロンプトキャッシュを最適化できるよう、`x-sente-session`ヘッダーを含める

## 利用制限

Sente Goには以下の基本制限が含まれています：
