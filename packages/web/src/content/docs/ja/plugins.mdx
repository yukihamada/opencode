---
title: プラグイン
description: Sente を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Sente を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Sente のデフォルトの動作を変更したりできます。
...
- `.sente/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/sente/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/sente/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/sente/sente.json`)
2. プロジェクト設定 (`sente.json`)
3. グローバルプラグインディレクトリ (`~/.config/sente/plugins/`)
4. プロジェクトプラグインディレクトリ (`.sente/plugins/`)
...
Sente は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Sente SDK クライアント。
...
ここでは、Sente を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://teai.io/sente/docs/sdk) を参照してください。
...
プラグインは Sente にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Sente が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Sente で使用できます。
