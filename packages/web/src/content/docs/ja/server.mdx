---
title: サーバー
description: HTTP 経由で Sente サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`sente serve` コマンドは、Sente クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `sente.local` |
...
HTTP 基本認証でサーバーを保護するには、`SENTE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `sente` になるか、`SENTE_SERVER_USERNAME` を設定してオーバーライドします。これは、`sente serve` と `sente web` の両方に当てはまります。
...
`sente` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Sente サーバーを使用して、プログラムで Sente と対話します。
:::

`sente serve` を実行してスタンドアロンサーバーを起動できます。すでに Sente TUI を実行している場合でも、`sente serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Sente [IDE](/docs/ide) プラグインによって使用されます。
