---
title: Complementos
description: Escriba sus propios complementos para extender Sente.
---

Los complementos le permiten extender Sente al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Sente.
...
- `.sente/plugins/` - Complementos a nivel de proyecto
- `~/.config/sente/plugins/` - Complementos globales
...
```json title="sente.json"
{
  ...
  "plugin": ["sente-helicone-session", "sente-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/sente/node_modules/`.
...
1. Configuración global (`~/.config/sente/sente.json`)
2. Configuración del proyecto (`sente.json`)
3. Directorio global de complementos (`~/.config/sente/plugins/`)
4. Directorio de complementos del proyecto (`.sente/plugins/`)
...
Sente ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".sente/plugins/my-plugin.ts"
...
```js title=".sente/plugins/example.js"
...
- `client`: Un cliente SDK sente para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar sente.
...
```js title=".sente/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "sente"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Sente, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que sente lea archivos `.env`:

```javascript title=".sente/plugins/env-protection.js"
...
```javascript title=".sente/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a sente:

```ts title=".sente/plugins/custom-tools.ts"
import { type Plugin, tool } from "@sente-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que sente puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para sente junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://teai.io/sente/docs/sdk) para obtener más detalles.
...
```ts title=".sente/plugins/compaction.ts"
import type { Plugin } from "@sente-ai/plugin"
...
```ts title=".sente/plugins/custom-compaction.ts"
import type { Plugin } from "@sente-ai/plugin"
