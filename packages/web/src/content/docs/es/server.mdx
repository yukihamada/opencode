---
title: Servidor
description: Interactuar con el servidor sente a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `sente serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente sente puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `sente.local` |
...
Configure `SENTE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `sente`, o configure `SENTE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `sente serve` como a `sente web`.
...
Cuando ejecuta `sente`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor sente para interactuar con sente mediante programación.
:::

Esta arquitectura permite que sente admita múltiples clientes y le permite interactuar con sente mediante programación.

Puede ejecutar `sente serve` para iniciar un servidor independiente. Si tienes el
sente TUI ejecutándose, `sente serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Sente [IDE](/docs/ide).
...
El servidor sente expone las siguientes API.
