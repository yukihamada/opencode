...
Las herramientas permiten que LLM realice acciones en su código base. Sente viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Sente.
...
:::note
Esta herramienta solo está disponible cuando `SENTE_EXPERIMENTAL_LSP_TOOL=true` (o `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Sente o cuando la variable de entorno `SENTE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Sente:

```bash
SENTE_ENABLE_EXA=1 sente
```
