...
Las habilidades del agente permiten a Sente descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Sente busca estas ubicaciones:

- Configuración del proyecto: `.sente/skills/<name>/SKILL.md`
- Configuración global: `~/.config/sente/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Sente sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.sente/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.sente/skills/git-release/SKILL.md` así:
...
Sente enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `sente.json`:
...
**Para agentes integrados** (en `sente.json`):
