...
description: Cree herramientas que LLM pueda llamar en sente.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de sente como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.sente/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/sente/tools/`.
...
```ts title=".sente/tools/database.ts" {1}
...
```ts title=".sente/tools/project.ts" {8}
import { tool } from "@sente-ai/plugin"
...
```python title=".sente/tools/add.py"
import sys
...
```ts title=".sente/tools/python-add.ts" {10}
import { tool } from "@sente-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".sente/tools/add.py")
