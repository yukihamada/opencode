...
Con Sente puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Sente usa nuestro propio tema `sente`.
...
Sente viene con varios temas integrados.
...
- Quiere que Sente coincida con la apariencia de su terminal
...
Sente admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/sente/themes/*.json` o `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.sente/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.sente/themes/*.json`
