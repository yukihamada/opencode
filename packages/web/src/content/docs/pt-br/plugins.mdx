---
title: Plugins
description: Escreva seus próprios plugins para estender o sente.
---

Plugins permitem que você estenda o sente conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do sente.
...
- `.sente/plugins/` - Plugins em nível de projeto
- `~/.config/sente/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/sente/node_modules/`.
...
1. Configuração global (`~/.config/sente/sente.json`)
2. Configuração do projeto (`sente.json`)
3. Diretório de plugins global (`~/.config/sente/plugins/`)
4. Diretório de plugins do projeto (`.sente/plugins/`)
...
O sente executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do sente para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o sente.
...
:::note
Se você estiver usando o aplicativo desktop sente, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o sente de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao sente:
...
O helper `tool` cria uma ferramenta personalizada que o sente pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o sente junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://teai.io/sente/docs/sdk) para detalhes.
