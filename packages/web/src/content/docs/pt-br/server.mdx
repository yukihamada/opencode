---
title: Servidor
description: Interaja com o servidor sente via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `sente serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente sente pode usar.
...
```bash
sente serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `sente.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
sente serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `SENTE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `sente`, ou defina `SENTE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `sente serve` quanto ao `sente web`.

```bash
SENTE_SERVER_PASSWORD=your-password sente serve
```
...
Quando você executa `sente`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor sente para interagir com o sente programaticamente.
:::

Essa arquitetura permite que o sente suporte múltiplos clientes e permite que você interaja com o sente programaticamente.

Você pode executar `sente serve` para iniciar um servidor autônomo. Se você tiver o TUI do sente em execução, `sente serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do sente [IDE](/docs/ide).
...
O servidor sente expõe as seguintes APIs.
