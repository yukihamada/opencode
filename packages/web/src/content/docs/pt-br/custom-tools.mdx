...
description: Crie ferramentas que o LLM pode chamar no sente.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do sente, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.sente/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/sente/tools/`.
