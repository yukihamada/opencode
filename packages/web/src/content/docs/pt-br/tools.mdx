...
As ferramentas permitem que o LLM execute ações em sua base de código. O sente vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no sente.
...
:::note
Esta ferramenta está disponível apenas quando `SENTE_EXPERIMENTAL_LSP_TOOL=true` (ou `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor sente ou quando a variável de ambiente `SENTE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o sente:
