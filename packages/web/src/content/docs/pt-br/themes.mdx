...
Com o sente, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o sente usa nosso próprio tema `sente`.
...
O sente vem com vários temas embutidos.
...
- Querem que o sente corresponda à aparência do seu terminal
...
O sente suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/sente/themes/*.json` ou `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.sente/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.sente/themes/*.json`
