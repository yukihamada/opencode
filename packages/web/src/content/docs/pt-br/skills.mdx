...
As habilidades do agente permitem que o sente descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O sente pesquisa nesses locais:

- Configuração do projeto: `.sente/skills/<name>/SKILL.md`
- Configuração global: `~/.config/sente/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o sente sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.sente/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.sente/skills/git-release/SKILL.md` assim:
...
O sente lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `sente.json`:
...
**Para agentes embutidos** (em `sente.json`):
