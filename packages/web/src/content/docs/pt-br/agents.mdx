...
Existem dois tipos de agentes no sente; agentes primários e subagentes.
...
sente vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Sente vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Sente vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Sente, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `sente.json`:
...
- Global: `~/.config/sente/agents/`
- Por projeto: `.sente/agents/`

```markdown title="~/.config/sente/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o sente usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do sente quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do sente usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Sente Zen](/docs/zen), você usaria `sente/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="sente.json" {3-6,9-12}
{
  "$schema": "https://teai.io/sente/config.json",
  "tools": {
    "write": true,
...
```json title="sente.json" {3-5,8-10}
{
  "$schema": "https://teai.io/sente/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `sente models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/sente).
:::
...
```markdown title="~/.config/sente/agents/docs-writer.md"
---
...
```markdown title="~/.config/sente/agents/security-auditor.md"
---
