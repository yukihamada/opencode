...
Sente Go는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있게 해주는 **월 $10**의 저비용 구독 서비스입니다.
...
Sente Go를 사용하면 **월 $10**으로 이러한 모델에 액세스할 수 있습니다.
...
Sente Go는 [Sente](https://teai.io/sente) 및 유사한 유형의 요청을 생성하는 다른 인기 코딩 에이전트와 함께 사용하도록 설계되었습니다.
...
3\. 프롬프트 캐싱을 최적화할 수 있도록 `x-sente-session` 헤더를 포함함

## 사용 한도

Sente Go에는 다음과 같은 기본 한도가 포함됩니다.
