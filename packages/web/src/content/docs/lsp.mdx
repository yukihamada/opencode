---
title: LSP Servers
description: Sente integrates with your LSP servers.
---

Sente can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Sente comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `SENTE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and sente opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your sente config.
