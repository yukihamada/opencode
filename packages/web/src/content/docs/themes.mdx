...
With Sente you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Sente uses our own `sente` theme.
...
Sente comes with several built-in themes.
...
- Want Sente to match their terminal's appearance
...
Sente supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/sente/themes/*.json` or `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.sente/themes/*.json`
4. **Current working directory** - Defined in `./.sente/themes/*.json`
