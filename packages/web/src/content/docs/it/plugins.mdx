---
title: Plugin
description: Scrivi plugin per estendere Sente.
---

I plugin ti permettono di estendere Sente agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Sente.
...
- `.sente/plugins/` - plugin a livello progetto
- `~/.config/sente/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/sente/node_modules/`.
...
1. Config globale (`~/.config/sente/sente.json`)
2. Config di progetto (`sente.json`)
3. Directory plugin globale (`~/.config/sente/plugins/`)
4. Directory plugin di progetto (`.sente/plugins/`)
...
Sente esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di sente per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere sente.
...
:::note
Se usi l'app desktop di Sente, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a sente di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a sente:
...
L'helper `tool` crea uno strumento personalizzato che sente può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in sente insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://teai.io/sente/docs/sdk) per i dettagli.
