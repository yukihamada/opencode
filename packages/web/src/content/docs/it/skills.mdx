...
Le skill degli agenti permettono a Sente di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Sente cerca in queste posizioni:

- Config di progetto: `.sente/skills/<name>/SKILL.md`
- Config globale: `~/.config/sente/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Sente risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.sente/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.sente/skills/git-release/SKILL.md` cosi':
...
Sente elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `sente.json`:
...
**Per agenti integrati** (in `sente.json`):
