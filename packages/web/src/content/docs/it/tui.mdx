---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Sente.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sente fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Sente avvia la TUI per la directory corrente.
...
Quando usi la TUI di Sente, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Sente. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `sente.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Sente attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `sente.default`.
