...
Sente Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Sente. Ti abboni a Sente Go e
...
Sente Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Sente Go funziona come qualsiasi altro provider in Sente.

1. Accedi a **<a href={console}>Sente Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Sente Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Sente Go.
:::
...
Sente Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Sente config
utilizza il formato `sente-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`sente-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Sente.
