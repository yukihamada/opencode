...
Sente Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.
...
Sente Go ti dà accesso a questi modelli a **10 $/mese**.
...
Sente Go è progettato per essere utilizzato con [Sente](https://teai.io/sente) e altri agenti di programmazione popolari che generano tipi di richieste simili.
...
3\. includa l'header `x-sente-session` in modo da consentirci di ottimizzare il caching dei prompt

## Limiti di utilizzo

Sente Go include i seguenti limiti di base:
