...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `SENTE_SERVER_USERNAME` o `opencode`) |
...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `sente auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `SENTE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `SENTE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| `SENTE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
