...
Puoi aggiungere strumenti esterni a Sente usando il _Model Context Protocol_, o MCP. Sente supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Sente Config](https://teai.io/sente/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/sente`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Sente gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Sente:
...
Se il server richiede autenticazione, Sente ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `sente mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Sente memorizzera' i token in modo sicuro in `~/.local/share/sente/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Sente insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Sente come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Sente al tuo account Sentry.
