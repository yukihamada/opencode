...
Du kan legge til eksterne verktøy i Sente ved å bruke _Model Context Protocol_ eller MCP. Sente støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Sente Config](https://teai.io/sente/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="sente.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/sente`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Sente håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Sente:
...
Hvis serveren krever autentisering, vil Sente be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `sente mcp auth <server-name>`.
...
```bash
sente mcp auth my-oauth-server
```
...
```bash
sente mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Sente lagre tokens sikkert i `~/.local/share/sente/mcp-auth.json`.
...
sente mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Sente, sammen med innebygde verktøy. Så du kan administrere dem gjennom Sente-konfigurasjonen som alle andre verktøy.
...
```json title="sente.json" {11, 14-18}
{
  "$schema": "https://teai.io/sente/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
sente mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Sente til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Sente-samtale oppført som en separat økt.
