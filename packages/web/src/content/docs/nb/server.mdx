---
title: Tjener
description: Samhandle med Sente-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `sente serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Sente-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `sente.local` |
...
Sett `SENTE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `sente` som standard, eller sett `SENTE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `sente serve` og `sente web`.
...
Når du kjører `sente` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Sente-serveren til å samhandle med Sente programmatisk.
:::

Denne arkitekturen lar Sente støtte flere klienter og lar deg samhandle med Sente programmatisk.

Du kan kjøre `sente serve` for å starte en frittstående server. Hvis du har
Sente TUI kjører, vil `sente serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Sente [IDE](/docs/ide) plugins.
...
sente-serveren viser følgende APIer.
