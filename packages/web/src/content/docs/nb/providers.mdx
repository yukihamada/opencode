---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Sente.
---

import config from "../../../../config.mjs"
export const console = config.console

Sente bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Sente-konfigurasjonen.
...
i `~/.local/share/sente/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Sente
konfig.
...
Sente Zen er en liste over modeller levert av Sente-teamet som har vært
testet og verifisert for å fungere godt med Sente. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Sente Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Sente Zen` og gå til [teai.io/sente/auth](https://teai.io/sente/zen).
...
Det fungerer som alle andre leverandører i Sente og er helt valgfritt å bruke.

---

## Sente Go

Sente Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Sente-teamet som har vært
testet og verifisert for å fungere godt med Sente.

1. Kjør kommandoen `/connect` i TUI, velg `Sente Go`, og gå til [teai.io/sente/auth](https://teai.io/sente/zen).
...
Slik bruker du Amazon Bedrock med Sente:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Sente:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `sente.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i sente støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere sente til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Sente skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Sente-konfigurasjonen.
...
for å lagre token i Sente auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Sente bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Sente
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`sente.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [sente-gitlab-auth](https://www.npmjs.com/package/sente-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `sente.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Sente:
...
Slik bruker du Google Vertex AI med Sente:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Sente, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [sente-helicone-session](https://github.com/H2Shami/sente-helicone-session) plugin for automatisk å logge hver Sente-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Sente-samtale oppført som en separat økt.
...
Du kan konfigurere Sente for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Sente for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Sente for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Sente. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/sente) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Sente:
...
3. Kopier API-nøkkelen for bruk i Sente.
...
6. **Viktig**: Før du bruker skymodeller i Sente, må du hente modellinformasjonen lokalt:
...
Sente Zen er en liste over testede og verifiserte modeller levert av Sente-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Sente Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Sente Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Sente API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Sente-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Sente-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Sente:
...
Du kan også tilpasse modeller gjennom Sente-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Sente. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `sente.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Sente forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `sente auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Sente-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Sente-konfigurasjonen.
