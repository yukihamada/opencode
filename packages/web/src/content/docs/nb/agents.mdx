...
Det er to typer agenter i Sente; primære agenter og underagenter.
...
Sente kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Sente kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Sente kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Sente sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `sente.json`:
...
- Globalt: `~/.config/sente/agents/`
- Per prosjekt: `.sente/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Sente modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Sente-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Sente-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Sente Zen](/docs/zen), vil du bruke `sente/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `sente models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/sente).
:::
