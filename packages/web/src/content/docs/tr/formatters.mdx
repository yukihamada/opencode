...
description: sente dile özgü biçimlendiriciler kullanır.
---

sente, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

sente, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, sente bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

sente bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri sente yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
