...
Ajan becerileri, sente'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
sente su konumlari tarar:

- Proje konfigurasyonu: `.sente/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/sente/skills/<name>/SKILL.md`
...
Proje yerel yollarinda sente, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.sente/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.sente/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
sente, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`sente.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`sente.json` icinde):
