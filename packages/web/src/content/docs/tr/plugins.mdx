---
title: Eklentiler
description: Sente'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Sente'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Sente'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.sente/plugins/` - Proje düzeyinde eklentiler
- `~/.config/sente/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/sente/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/sente/sente.json`)
2. Project config (`sente.json`)
3. Global eklenti dizini (`~/.config/sente/plugins/`)
4. Proje eklenti dizini (`.sente/plugins/`)
...
Sente bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Sente SDK istemcisi.
...
Sente'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Sente masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Sente'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Sente'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Sente'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Sente'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://teai.io/sente/docs/sdk) bakın.
