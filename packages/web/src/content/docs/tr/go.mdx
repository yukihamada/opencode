...
Sente Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Sente'daki diğer sağlayıcılar gibi çalışır. Sente Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Sente'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Sente Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Sente Go, Sente'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Sente Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Sente Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Sente Go'ya abone olabilir.
:::
...
Sente Go aşağıdaki limitleri içerir:
...
Sente yapılandırmanızdaki [model id](/docs/config/#models) formatı `sente-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `sente-go/kimi-k3` kullanmalısınız.
...
Sente Go'yu şu amaçlarla oluşturduk:
...
4. Sente ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
