...
Sente Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.
...
Sente Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.
...
Sente Go, [Sente](https://teai.io/sente) ve benzer türde istekler üreten diğer popüler kodlama aracılarıyla kullanılmak üzere tasarlanmıştır.
...
3\. istem önbelleğe almayı optimize edebilmemiz için `x-sente-session` başlığını içerdiğinden emin olun

## Kullanım limitleri

Sente Go aşağıdaki temel limitleri içerir:
