...
sente ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak sente kendi `sente` temasını kullanır.
...
sente birden fazla yerleşik temayla gelir.
...
- sente'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
sente, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/sente/themes/*.json` veya `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.sente/themes/*.json`
4. **Geçerli çalışma dizini** - `./.sente/themes/*.json`
