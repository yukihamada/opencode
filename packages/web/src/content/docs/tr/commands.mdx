...
`.sente/commands/test.md` oluştur:
...
sente işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

sente [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/sente/commands/`
- Proje başına: `.sente/commands/`
...
sente, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
