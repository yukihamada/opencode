...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `SENTE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `SENTE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `opencode serve` hem de `opencode web` için geçerlidir.

```bash
SENTE_SERVER_PASSWORD=your-password opencode serve
```
