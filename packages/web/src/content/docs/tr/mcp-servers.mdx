...
_Model Bağlam Protokolü_ veya MCP kullanarak sente'a harici araçlar ekleyebilirsiniz. sente hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [sente Config](https://teai.io/sente/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/sente` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
sente, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde sente şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa sente, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `sente mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından sente, belirteçleri `~/.local/share/sente/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra sente'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi sente yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve sente'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
