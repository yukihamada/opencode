...
sente'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
sente, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Sente, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Sente iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `sente.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse sente modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global sente yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
sente yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Sente Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `sente/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `sente models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/sente).
:::
