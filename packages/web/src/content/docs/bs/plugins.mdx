---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Sente.
---

Dodaci vam omogućavaju da proširite Sente spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Sente.
...
- `.sente/plugins/` - Dodaci na nivou projekta
- `~/.config/sente/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/sente/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/sente/sente.json`)
2. Konfiguracija projekta (`sente.json`)
3. Globalni direktorij dodataka (`~/.config/sente/plugins/`)
4. Direktorij dodataka projekta (`.sente/plugins/`)
...
Sente pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Sente SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Sente.
...
Spriječite sente da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u sente:
...
Pomoćnik `tool` kreira prilagođeni alat koji sente može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za sente zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://teai.io/sente/docs/sdk) za detalje.
