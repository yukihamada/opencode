---
title: Server
description: Komunicirajte s sente serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `sente serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji sente klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `sente.local` |
...
Postavite `SENTE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `sente`, ili postavite `SENTE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `sente serve` i za `sente web`.
...
Kada pokrenete `sente`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite sente server za programsku interakciju sa sente-om.
:::

Ova arhitektura omogucava sente podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `sente serve` da startate standalone server. Ako je sente TUI vec pokrenut, `sente serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Sente [IDE](/docs/ide) pluginovi.
...
sente server izlaže sljedece API-je.
