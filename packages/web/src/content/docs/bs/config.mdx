---
title: Konfiguracija
description: Korištenje Sente JSON konfiguracije.
---

Možete konfigurirati Sente koristeći JSON konfiguracijski fajl.

---

## Format

Sente podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/sente`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/sente/sente.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SENTE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`sente.json` u projektu) - postavke specifične za projekat
5. **`.sente` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SENTE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.sente` i `~/.config/sente` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/sente` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Sente konfiguraciju u `~/.config/sente/sente.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/sente/tui.json`.
...
Dodajte `sente.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Sente pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SENTE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SENTE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.sente` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.sente`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`teai.io/sente/config.json`**](https://teai.io/sente/config.json).

TUI konfiguracija koristi [**`teai.io/sente/tui.json`**](https://teai.io/sente/tui.json).
...
Koristite `SENTE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `sente.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `sente serve` i `sente web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Sente server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `sente.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Sente konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Sente pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/sente/agents/` ili `.sente/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Sente će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`sente run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/sente/commands/` ili `.sente/commands/`. [Saznajte više ovdje](/docs/commands).
...
Sente će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Sente **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Sente sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.sente/plugins/` ili `~/.config/sente/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Sente da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
