...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Sente dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Sente.
...
:::note
Ovaj alat je dostupan samo kada je `SENTE_EXPERIMENTAL_LSP_TOOL=true` (ili `SENTE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Sente provajdera ili kada je varijabla `SENTE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Sente:
