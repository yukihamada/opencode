---
title: LSP serveri
description: Sente se integriše sa vašim LSP serverima.
---

Sente se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Sente dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `SENTE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i sente otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj sente konfiguraciji.
