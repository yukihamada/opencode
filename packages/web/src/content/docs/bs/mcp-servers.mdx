...
Možete dodati vanjske alate u Sente koristeći _Model Context Protocol_, ili MCP. Sente podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Sente Config](https://teai.io/sente/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/sente`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Sente automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Sente će:
...
Ako server zahtijeva autentifikaciju, Sente će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `sente mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Sente sigurno čuva tokene u `~/.local/share/sente/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Sente, zajedno s ugrađenim alatima. Možete njima upravljati kroz Sente konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Sente sa vašim Sentry nalogom.
