...
Da biste otklonili probleme s Sente, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sente\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `sente --log-level DEBUG`.

---

## Pohrana

Sente pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/sente/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sente`
...
Sente Desktop pokreće lokalni Sente server (`sente-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Sente` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/sente/sente.jsonc` (ili `~/.config/sente/sente.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/sente/sente.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sente\sente.jsonc`
...
Sente također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/sente/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sente\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Sente mogao ponovo izgraditi.

1. Potpuno zatvorite Sente Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/sente`
- **Linux**: obrišite `~/.cache/sente` (ili pokrenite `rm -rf ~/.cache/sente`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\sente`

3. Ponovo pokrenite Sente Desktop.

---

### Rješavanje problema sa vezom na serveru

Sente Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `sente.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SENTE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SENTE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Sente Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Sente karakteristikama.

---

### Obavještenja se ne prikazuju

Sente Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Sente u postavkama vašeg OS-a, i
...
1. Zatvorite Sente Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Sente Desktop aplikacije):

- `sente.settings.dat` (URL zadanog servera za desktop)
- `sente.global.dat` i `sente.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `sente upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `sente models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\sente`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Sente dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\sente`

2. Ponovo pokrenite Sente da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Sente da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Sente će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
