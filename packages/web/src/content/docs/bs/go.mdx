...
Sente Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Sente-u. Pretplatite se na Sente Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Sente.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Sente-om.
:::
...
Sente Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Sente Go radi kao bilo koji drugi provajder u Sente-u.

1. Prijavite se na **<a href={console}>Sente Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Sente Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Sente Go.
:::
...
Sente Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Sente konfiguraciji
koristi format `sente-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`sente-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Sente Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Sente koristite i bilo kojeg drugog provajdera.
