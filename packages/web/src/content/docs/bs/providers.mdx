---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Sente.
---

import config from "../../../../config.mjs"
export const console = config.console

Sente koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Sente konfiguraciji.
...
u `~/.local/share/sente/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Sente
config.
...
Sente Zen je lista modela koje je obezbedio Sente tim koji su bili
testirano i potvrđeno da dobro radi sa Sente. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Sente Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite sente i idite na [teai.io/sente/auth](https://teai.io/sente/auth).
...
Radi kao i svaki drugi provajder u Sente i potpuno je opcionalan za korištenje.

---

## Sente Go

Sente Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Sente tim i koji su testirani i verificirani da dobro rade s Sente-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Sente Go` i idite na [teai.io/sente/auth](https://teai.io/sente/zen).
...
Da biste koristili Amazon Bedrock s Sente:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete sente:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `sente.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Sente.
:::
...
Možete konfigurirati sente za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi sente ispravno radio.
   :::
...
Također možete dodati modele kroz svoju sente konfiguraciju.
...
da pohrani token u sente auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Sente koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Sente
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`sente.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [sente-gitlab-auth](https://www.npmjs.com/package/sente-gitlab-auth).

##### Konfiguracija

Prilagodite putem `sente.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s sente:
...
Za korištenje Google Vertex AI s Sente:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće sente.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem sentea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [sente-helicone-session](https://github.com/H2Shami/sente-helicone-session) da automatski prijavite svaki Sente razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Sente razgovor naveden kao zasebna sesija.
...
Možete konfigurirati sente za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati sente za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati sente za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Sente. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/sente) za detalje.
:::
...
Da biste koristili Ollama Cloud s Sente:
...
3. Kopirajte API ključ za korištenje u Sente.
...
6. **Važno**: Prije upotrebe modela oblaka u Sente, morate lokalno povući informacije o modelu:
...
Sente Zen je lista testiranih i verifikovanih modela koju je obezbedio Sente tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Sente Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Sente Zen**.

```txt
   /connect
```

3. Unesite svoj Sente API ključ.
...
Također možete dodati dodatne modele putem vaše sente konfiguracije.
...
5. Također ih možete prilagoditi putem vaše sente konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Senteom:
...
Također možete prilagoditi modele kroz svoju sente konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s sente-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `sente.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Sente da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `sente auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Sente konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj sente konfiguraciji.
