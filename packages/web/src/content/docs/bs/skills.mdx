...
Agent skills omogucavaju Sente da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Sente pretrazuje ove lokacije:

- Konfiguracija projekta: `.sente/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/sente/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Sente ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.sente/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/sente/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.sente/skills/git-release/SKILL.md` ovako:
...
Sente navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `sente.json`:
...
**Za ugradene agente** (u `sente.json`):
