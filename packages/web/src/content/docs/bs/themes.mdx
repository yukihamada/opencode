...
U Sente mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Sente koristi nasu `sente` temu.
...
Sente dolazi sa vise ugradenih tema.
...
- Zele da Sente odgovara izgledu njihovog terminala
...
Sente podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/sente/themes/*.json` ili `$XDG_CONFIG_HOME/sente/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.sente/themes/*.json`
4. **Trenutni radni direktorij** - `./.sente/themes/*.json`
