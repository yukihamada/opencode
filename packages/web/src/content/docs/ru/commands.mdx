...
Создайте `.sente/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию sente или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем sente [config](/docs/config):
...
- Глобальный: `~/.config/sente/commands/`
- Для каждого проекта: `.sente/commands/`
...
sente включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
