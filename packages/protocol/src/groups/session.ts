...
import { PromptInput } from "@sente-ai/schema/prompt-input"
import { Session } from "@sente-ai/schema/session"
import { Project } from "@sente-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@sente-ai/schema/schema"
...
import { SessionEvent } from "@sente-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Sente process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Sente process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
