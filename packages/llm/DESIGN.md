...
> current private `@sente-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@sente-ai/ai`
...
- Secondary audience: Sente and other durable agent runtimes
...
import { LLM } from "@sente-ai/ai"
import { OpenAI } from "@sente-ai/ai/providers/openai"
...
```ts
import { Message } from "@sente-ai/ai"
...
import { LLM, Tool } from "@sente-ai/ai"
...
Sente and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@sente-ai/ai/promise"
import { OpenAI } from "@sente-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@sente-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@sente-ai/ai/provider"
...
@sente-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@sente-ai/ai/providers/openai
@sente-ai/ai/providers/anthropic
@sente-ai/ai/providers/google
...
```
...
Sente should migrate to `generateTurn` / `streamTurn`, preserving its durable
