...
The sente server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "sente" instance like in that post i showed
- sente instance has stuff like `sente.session.prompt()` or
  `sente.tool.register({...})`
