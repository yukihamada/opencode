...
Both `action` and `resource` use sente's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.sente` files is intentionally deferred until `.sente` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether sente permits an operation through its managed execution paths.
