# Remove `packages/sente/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/sente/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the sente legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/sente/src/storage/db.ts` are concentrated in 22 source files:

- `packages/sente/src/account/repo.ts`
- `packages/sente/src/cli/cmd/db.ts`
- `packages/sente/src/cli/cmd/import.ts`
- `packages/sente/src/cli/cmd/stats.ts`
- `packages/sente/src/control-plane/workspace.ts`
- `packages/sente/src/index.ts`
- `packages/sente/src/node.ts`
- `packages/sente/src/permission/index.ts`
- `packages/sente/src/project/project.ts`
- `packages/sente/src/server/projectors.ts`
- `packages/sente/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/sente/src/server/shared/fence.ts`
- `packages/sente/src/session/message-v2.ts`
- `packages/sente/src/session/projectors.ts`
- `packages/sente/src/session/prompt.ts`
- `packages/sente/src/session/session.ts`
- `packages/sente/src/session/todo.ts`
- `packages/sente/src/share/share-next.ts`
- `packages/sente/src/storage/db.ts`
- `packages/sente/src/sync/index.ts`
- `packages/sente/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy sente database wrapper; `packages/sente/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the sente wrapper.
...
Status: Completed. `SyncEvent` and the sente projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy sente database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy sente database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy sente database wrapper.
...
All migration groups are complete or superseded. `packages/sente/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No sente data-migration group remains.

The previous sente `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate sente data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/sente`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/sente/src`
- `bun typecheck` from `packages/sente`
- Relevant package tests from `packages/sente`, not the repo root
