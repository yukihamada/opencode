...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an sente storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/sente` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No sente paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Sente-specific path/channel selection stays in `packages/sente`.

## Sente Adoption Notes

These are not package requirements, but they matter for the later sente adoption PR.

The current `packages/sente/src/storage/db.ts` has two non-obvious semantics that the sente wrapper must preserve when it consumes this adapter:
...
The sente wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving sente to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.
...
4. Add `@sente-ai/effect-drizzle-sqlite` as a dependency of `packages/sente`.
5. Port `packages/sente/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus sente-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of sente's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then sente can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay sente-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/sente`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not sente domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/sente` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing sente's current database runtime.
