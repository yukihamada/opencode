...
Move the canonical Sente terminal application from
`packages/sente/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/sente`,
`packages/cli`, or `@sente-ai/core`.

The SDK is the TUI's Sente boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/sente`.
- Do not replace private `packages/sente` imports with `@sente-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@sente-ai/tui`.
...
- Sente domain data displayed by the TUI
...
```text
packages/sente/src/cli/cmd/tui
```

Its private dependency on `packages/sente` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/sente/src/*`.
...
Status: Completed. The private `@sente-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@sente-ai/tui`.
...
- The package has no dependency on `sente`, `@sente-ai/cli`, or
  `@sente-ai/core`.
...
primitives now live in `@sente-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@sente-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Sente globals or process state;
...
Replace process-global Sente state with resolved TUI inputs.
...
`@sente-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/sente` initially.
...
canonical in `@sente-ai/tui`. Configured references resolve through the new
...
adapters rather than Sente domain implementations.

Make the SDK the only Sente domain boundary used by the TUI.
...
- Domain-facing TUI code imports Sente data and operations only from
  `@sente-ai/sdk`.
...
`@sente-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/sente`.
...
- The package has no imports from `packages/sente`, `packages/cli`, or
  `@sente-ai/core`.
...
public `@sente-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/sente`.
- Keep the legacy embedded worker and server startup in `packages/sente`.
- Change those adapters to load config, create transport inputs, and call the
  public `@sente-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/sente`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@sente-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@sente-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Sente domain state.
...
cd packages/sente && bun typecheck
...
rg '@sente-ai/core|packages/sente|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/sente/src packages/sente/test
```
...
- Build the current-platform `packages/sente` binary.
