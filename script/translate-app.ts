...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "sente/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".sente/glossary/zh-cn.md"
  if (locale === "zht") return ".sente/glossary/zh-tw.md"
  return `.sente/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Sente did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Sente returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Sente session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Sente runs for 'all' (default: 4)
      --model <provider/id>  Sente model (default: sente/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Sente
...
`)
  ...
  if (failed.length) console.error(`\nSente failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.SENTE_DISABLE_PROJECT_CONFIG = "1"
  env.SENTE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["sente", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.SENTE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["sente", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.SENTE_CONFIG
  delete env.SENTE_CONFIG_DIR
  delete env.SENTE_CONFIG_CONTENT
  delete env.SENTE_PERMISSION
  delete env.SENTE_AUTO_SHARE
  return env
}
