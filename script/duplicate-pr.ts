...
import { createSente } from "@sente-ai/sdk"
...
`)
  ...
  const sente = await createSente({ port: 0 })
  ...
  try {
    ...
    const session = await sente.client.session.create()
    const result = await sente.client.session
      .prompt({
