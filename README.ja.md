...
# パッケージマネージャー
npm i -g sente-ai@latest        # bun/pnpm/yarn でもOK
scoop install sente             # Windows
choco install sente             # Windows
brew install anomalyco/tap/sente # macOS と Linux（推奨。常に最新）
brew install sente              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S sente            # Arch Linux (Stable)
paru -S sente-bin               # Arch Linux (Latest from AUR)
mise use -g sente               # どのOSでも
nix run nixpkgs#sente           # または github:anomalyco/sente で最新 dev ブランチ
```
...
Sente はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/sente/releases) から直接ダウンロードするか、[teai.io/sente/download](https://teai.io/sente/download) を利用してください。
...
1. `$SENTE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.sente/bin` - デフォルトのフォールバック
...
Sente には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://teai.io/sente/docs/agents) の詳細はこちら。

### ドキュメント

Sente の設定については [**ドキュメント**](https://teai.io/sente/docs) を参照してください。

### コントリビュート

Sente に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Sente の上に構築する

Sente に関連するプロジェクトで、名前に "sente"（例: "sente-dashboard" や "sente-mobile"）を含める場合は、そのプロジェクトが Sente チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/sente) | [X.com](https://x.com/sente)
